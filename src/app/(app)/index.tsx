...
// Conditionally import Didcomm - only available in development builds, not Expo Go
let Didcomm: any = null;
try {
  if (__DEV__) {
    Didcomm = require("didcomm-sdk").default;
  }
} catch (error) {
  // Didcomm not available (e.g., in Expo Go)
  console.log("Didcomm module not available in Expo Go");
}
...
export default function Index() {
  ...
  useEffect(() => {
    const initDidcomm = async () => {
      // Only initialize Didcomm if it's available (development build, not Expo Go)
      if (!Didcomm) {
        console.log("Didcomm not available - skipping initialization");
        return;
      }
      
      try {
        const result = await Didcomm.helloWorld();
        console.log("Didcomm result:", result);
      } catch (err) {
        console.error("Didcomm initialization error:", err);
      }
    };
